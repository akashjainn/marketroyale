...
// List contests
contestsRouter.get('/', requireDatabase, async (_req, res) => {
  const contests = await prisma!.contest.findMany({ orderBy: { startAt: 'desc' as const }, take: 50 });
  res.json({ contests });
});

// Get single contest
contestsRouter.get('/:id', requireDatabase, async (req, res) => {
...
contestsRouter.post('/', requireDatabase, async (req, res) => {
...
contestsRouter.post('/:id/publish', requireDatabase, async (req, res) => {
...
contestsRouter.post('/:id/join', requireDatabase, async (req, res) => {
  const contest = await prisma!.contest.findUnique({ where: { id: req.params.id } });
  if (!contest) return res.status(404).json({ error: 'Not found' });
...
contestsRouter.post('/entries/:entryId/picks', requireDatabase, async (req, res) => {
...
contestsRouter.get('/:id/leaderboard', requireDatabase, async (req, res) => {
