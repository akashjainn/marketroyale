...
// List contests
exports.contestsRouter.get('/', async (_req, res) => {
    const contests = await prisma_1.prisma.contest.findMany({ orderBy: { startAt: 'desc' }, take: 50 });
    res.json({ contests });
});
// Get single contest
exports.contestsRouter.get('/:id', async (req, res) => {
...
exports.contestsRouter.post('/', async (req, res) => {
...
exports.contestsRouter.post('/:id/publish', async (req, res) => {
...
exports.contestsRouter.post('/:id/join', async (req, res) => {
    const contest = await prisma_1.prisma.contest.findUnique({ where: { id: req.params.id } });
    if (!contest)
        return res.status(404).json({ error: 'Not found' });
...
exports.contestsRouter.post('/entries/:entryId/picks', async (req, res) => {
...
exports.contestsRouter.get('/:id/leaderboard', async (req, res) => {
